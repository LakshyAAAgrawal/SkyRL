...
DATA_DIR="/home/ubuntu/SkyRL/aime"
...
TRAIN_BATCH_SIZE=32
MICRO_TRAIN_BSZ_PERGPU=4
MICRO_FWD_BSZ_PERGPU=12
...
# SEQ_PAR_SIZE=1
# trainer.policy.sequence_parallel_size=$SEQ_PAR_SIZE \
...
uv run --isolated --extra $INFERENCE_BACKEND -m skyrl_train.entrypoints.main_generate \
  ...
  trainer.micro_forward_batch_size_per_gpu=$MICRO_FWD_BSZ_PERGPU \
  ...
  trainer.export_path="$HOME/exports_test_$CUR_TIME" \
  trainer.ckpt_path="$HOME/ckpts/aime_ckpttest_$CUR_TIME" \
...
uv run --isolated --extra $INFERENCE_BACKEND -m skyrl_train.entrypoints.main_base \
  ...
  trainer.run_name="aime_test" \
  trainer.resume_mode=null \
  trainer.export_path="$HOME/exports_$CUR_TIME" \
  trainer.ckpt_path="$HOME/ckpts/aime_ckpt_$CUR_TIME" \
  trainer.dump_eval_results=true \
  $@